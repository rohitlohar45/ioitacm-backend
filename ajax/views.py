    ...
    if request.GET:
        if request.user.is_authenticated:
            if request.user.is_ec:
                ...
                # Message Formatting
                email_msg = email_msg.replace("[[Participant Name]]",participant.name)
                email_msg = email_msg.replace("[[Participant Student ID]]",participant.std_id)
                email_msg = email_msg.replace("[[Participant Email]]",participant.email)
                email_msg = email_msg.replace("[[Participant Phone]]",participant.phone)
                email_msg = email_msg.replace("[[Participant First Name]]",(participant.name.split())[0])
...
def check_perticipate_data(request):
    ...
    if request.GET:
        ...
        data['ok'] = False
        check = Participants_details.objects.filter(email=email).filter(event=event).count()
        if check>0:
...
def register_participant(request):
    ...
    if request.GET:
        ...
        if check>0:
            ...
            participants_details = Participants_details(name = name,std_id = std_id,email=email,phone=phone,event=event,club=club)
