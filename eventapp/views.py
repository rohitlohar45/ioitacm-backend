...
def participants(request):
    ...
    if temp['auth']:
        ...
        return render(request, 'club/participants.html',context)
    else:
        return redirect('login')


def participants_details(request,eventname):
    context = {}
    temp = ec_is_autenticate(request)
    if temp['auth']:
        context['club'] = temp['club']
        ...
        if check==0:
            ...
            context['participants'] = Participants_details.objects.filter(event = event)
            return render(request, 'club/participantsdetails.html',context)
    else:
        return redirect('login')
